...
from mmpose.apis import (inference_pose_lifter_model, extract_pose_sequence,
...
def main():
    ...
    if args.only_second_stage:
        from mmpose.apis.inference import bbox_xywh2xyxy    # wyh `_xywh2xyxy` to `bbox_xywh2xyxy`
    ...
    for i, pose_det_results in enumerate(
        ...
        # Note that the pose_det_results 被视为单姿态序列
        # --------------- wyh 23.1.11 ADD ---------------------------------------------------
        my_len = int(args.pose_lifter_config.rsplit('/', 1)[1].split('_')[2][0:2])
        pose_results_2d = extract_pose_sequence(pose_det_results_list, frame_idx=i,    # 填充序列
                                                causal=False, seq_len=my_len)
        # ------------------- END  ----------------------------------------------------------
        pose_lift_results = inference_pose_lifter_model(
            pose_lift_model,
            pose_results_2d=pose_results_2d,   # [pose_det_results] * my_len
            dataset=dataset,
            dataset_info=dataset_info,
            with_track_id=False,
            image_size=(1280, 720)
        ...
        for idx, res in enumerate(pose_lift_results):
            ...
            # only visualize the target frame  -- wyh 23.1.11  ---------------------------
