...
def convert_keypoint_definition(keypoints, pose_det_dataset,
    ...
    if pose_det_dataset == 'TopDownH36MDataset' and pose_lift_dataset == 'Body3DH36MDataset':
        return keypoints
    elif pose_det_dataset == 'TopDownArm9Dataset' and pose_lift_dataset == 'Body3DArm9Dataset':
        return keypoints
...
def main():
    ...
    for i, pose_det_results in enumerate(
        ...
        for idx, res in enumerate(pose_lift_results):
            ...
            if not args.save_csv:
                keypoints_3d = keypoints_3d[..., [0, 2, 1]]   # wyh anno (tempora)
                keypoints_3d[..., 0] = -keypoints_3d[..., 0]  # x = -x
                keypoints_3d[..., 2] = -keypoints_3d[..., 2]  # z = -y

            # ----------- for RobotApp change coordinate ---------------
            if args.save_csv:
                keypoints_3d = keypoints_3d[..., [2, 0, 1]]
                keypoints_3d[..., 0] = -keypoints_3d[..., 0]  # x = -x
                keypoints_3d[..., 2] = -keypoints_3d[..., 2]  # z = -y
                keypoints_3d[..., :] -= keypoints_3d[0, :]  # reduce BasePoint
                robotApp_pose.append(keypoints_3d.flatten())
            # ------------- END ----------------------------------------
        ...
        suffix = osp.join(args.out_video_root, osp.basename(args.video_path).split('.')[0])
        img_vis = vis_3d_pose_result(
            pose_lift_model,
            result=pose_lift_results_vis,
            img=video[i],
            dataset='Body3DArm9Dataset',  # wyh 23.1.11
            out_file=None,  # *************************  若想存为图片,则out_file赋值,注释下列保存视频code **********
            radius=args.radius,
    ...
    if args.save_csv:
        csvName = osp.join(args.out_video_root, osp.basename(args.video_path).split('.')[0]) + '.csv'
        with open(csvName, 'w', newline='') as cfile:
            writer = csv.writer(cfile)
            for iv in robotApp_pose:
                writer.writerow(iv)
