...
def inference_pose_lifter_model(model,
    ...
    if dataset_info is not None:
        ...
        if dataset == 'Body3DH36MDataset':
            ...
            bbox_scale = 400
        elif dataset == 'Body3DArm9Dataset':
            flip_pairs = [[1, 4], [2, 5], [3, 6], [7, 8]]
            bbox_center = np.array([[528, 427]], dtype=np.float32)
            bbox_scale = 400
        else:
...
def vis_3d_pose_result(model,
    ...
    if dataset_info is not None:
        ...
        elif dataset in 'Body3DArm9Dataset':
            # show the results
            skeleton = [[0, 1], [0, 4], [1, 2], [2, 3], [3, 7], [4, 5], [5, 6], [6, 8]]
            pose_link_color = palette[[9, 9, 16, 16, 16, 7, 7, 7]]
            pose_kpt_color = palette[[9, 16, 16, 16, 0, 0, 0, 16, 0]]
