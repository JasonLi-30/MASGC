...
class Kpt2dSviewRgbImgTopDownDataset(Dataset, metaclass=ABCMeta):
    ...
    def _report_metric(self,
                       res_file,
                       metrics,
                       pck_thr=0.016,  # wyh commonly set as 0.05, 0.1 or 0.2 etc. 0.03 -> error 50mm  0.016 -> error 29mm
                       pckh_thr=0.7,
                       auc_nor=30):
        """Keypoint evaluation.
        ...
        if 'PCK' in metrics:
            ...
            acc, pck, acc_part, pck_part = keypoint_pck_accuracy(outputs, gts, masks, pck_thr,
                                              threshold_bbox)
            info_str.append(('PCK', pck))
            info_str.append(('PCK-vis', pck_part))  # --------wyh 22.12.16 add -----------
            for ii in range(len(acc)):
                info_str.append(('acc_' + str(ii), acc[ii]))
                info_str.append(('acc_' + str(ii) + '-vis', acc_part[ii]))
