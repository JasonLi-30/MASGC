...
from ..necks import FPCA
from ..builder import BACKBONES
from .hrnet import Bottleneck, HRModule, HRNet
from .resnet import get_expansion
...
class HRFormerBlock(BaseModule):
    """High-Resolution Block for HRFormer.   【Block】
...
class HRFomerModule_FPCA(HRModule):
    def __init__(self,
                 num_branches,
                 block,
                 num_blocks,
        ...
        super().__init__(num_branches, block, num_blocks, num_inchannels,
                         num_channels, multiscale_output, with_cp, conv_cfg,
                         norm_cfg, upsample_cfg)
        self.fpn_lsj = FPCA(list(num_channels), out_channels=32, num_outs=len(num_channels))
    ...
    def forward(self, x):
        ...
        if not self.multiscale_output:
            x = self.fpn_lsj(x)
            return [x[0]] if x[0].shape[3] > x[3].shape[3] else [x[3]] 
...
class HRFormer_FPCA(HRNet):
    ...
    def __init__(self,
        ...
        self.stage4, pre_stage_channels = self._make_stage_fpn(
            self.stage4_cfg,
            num_channels,
