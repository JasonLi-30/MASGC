...
class LocalGraph(nn.Module):
    def __init__(self, adj, input_dim, output_dim, dropout=None):
        ...
        if num_joints == 9:
            ...
            store_2 = [13, 16] 
            joints_left = [11, 12, 13]
            joints_right = [14, 15, 16]
        else:
...
class GraphAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # x: (B, T, K, C) --> (B, C, T, K) 
        x = x.permute(0, 3, 1, 2)
        # x = self.relu(self.cat_bn(self.conv1d(self.relu(self.cat_bn(self.cat_conv(x))))) + residual)   # channel callback lsj
        # x = self.relu(self.cat_bn(self.cat_conv(x) + residual))   # channel callback wyh
        # x = self.relu(self.cat_conv(self.relu(self.cat_bn(x))) + residual)   # channel callback lsj--preactivation1
...
class TSGCN(BaseBackbone): 
    ...
    def __init__(self,
        ...
        arm9_skeleton = Graph_Skeleton(parents=[-1, 0, 1, 2, 0, 4, 5, 0, 7, 8, 9, 8, 11, 12, 8, 14, 15],  # 17
                                       joints_left=[11, 12, 13], joints_right=[14, 15, 16])
