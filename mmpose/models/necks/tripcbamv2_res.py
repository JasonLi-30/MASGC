...
class TriPCBAMv2_res(BaseModule):
    ...
    def __init__(self,
        ...
        super(TriPCBAMv2_res, self).__init__(init_cfg)
    ...
    def forward(self, inputs):
        ...
        outs_coord = [self.coord[i](outs_ca[i]) * outs_ca[i] for i in range(used_backbone_levels)]
        outs_sa = [self.sa[i](outs_ca[i]) * (outs_ca[i] + outs_coord[i])  for i in range(used_backbone_levels)]

        outs = []
        for i in range(used_backbone_levels):
            # weights_norm = torch.softmax(self.weight[2 * i: 2 * i + 2], dim=0)
            outs += [ outs_sa[i] + outs_ca[i] + laterals[i]]
