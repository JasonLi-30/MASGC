...
class TemporalRegressionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.wyh_conv2d = build_conv_layer(
            dict(type='Conv2d'), in_channels, 3, 1)
    ...
    def forward(self, x):
        """Forward function."""
        x = self._transform_inputs(x)  # Tensor(128,1024,1)  # (B, C=1024, T=1, K=9)
        ...
        # ------------- wyh 23.1.18 add for 2d train ----------------------
        if x.ndim == 4 and x.shape[2] == 1:
            output = self.wyh_conv2d(x)
