...
def pose_pck_accuracy(output, target, mask, thr=0.05, normalize=None):
    ...
    return keypoint_pck_accuracy(pred, gt, mask, thr, normalize)[0:3]
...
def keypoint_pck_accuracy(pred, gt, mask, thr, normalize):
    ...
    distances_all = _calc_distances(pred, gt, np.ones(mask.shape, dtype=bool), normalize)
    acc = np.array([_distance_acc(d, thr) for d in distances_all])
    valid_acc = acc[acc >= 0]
    cnt = len(valid_acc)
    avg_acc = valid_acc.mean() if cnt > 0 else 0

    # wyh 22.12.16
    distances_part = _calc_distances(pred, gt, mask, normalize)
    acc_part = np.array([_distance_acc(d, thr) for d in distances_part])
    valid_acc_part = acc_part[acc_part >= 0]
    cnt_part = len(valid_acc_part)
    avg_acc_part = valid_acc_part.mean() if cnt_part > 0 else 0

    return acc, avg_acc, acc_part, avg_acc_part
