...
def imshow_keypoints_3d(
    ...
    if show_img:
        ...
        # wyh 23.2.14
        # tiff_name = suffix + "_" + str(index) + '.tiff'
        # if index != -1 and index in [1, 31, 61, 91, 121, 151, 181, 211, 241, 271, 301, 331, 361, 391]:
        #     cv2.imwrite(tiff_name, img, ((int(cv2.IMWRITE_TIFF_RESUNIT), 2,
        #                                   int(cv2.IMWRITE_TIFF_XDPI), 96,
        #                                   int(cv2.IMWRITE_TIFF_XDPI), 96)))
        # --------- END -------------------------
    ...
    for idx, res in enumerate(pose_result):
        ...
        ax.set_zlim3d([-1.2, axis_limit - 1.2])  # [0, axis_limit]
        ax.set_aspect('auto')
        ...
        ax.set_xticks([-0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75])  # anno + scaleLine
        ax.set_yticks([-0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75])
        ax.set_zticks([-1, -0.75, -0.5, -0.25, 0, 0.25, 0.5])
        # ax.set_xticks([])
        # ax.set_yticks([])
        # ax.set_zticks([])
        ax.set_xticklabels([])  # anno
        ax.set_yticklabels([])
        ax.set_zticklabels([])
        ax.dist = axis_dist
        ...
        if not dummy and pose_kpt_color is not None:
            ...
            ax.scatter(  # 绘制散点图
                ...
                marker='o',
                color=_color[valid])
            # for q in range(len(pose_kpt_color)):
            #     ax.scatter(x_3d[valid][q], y_3d[valid][q], z_3d[valid][q], marker='o', color=_color[valid][q])
    ...
    # wyh 23.2.14 add for save svg
    svg_name = suffix + "_" + str(index) + '.svg'
    if index != -1 and index in [1, 31, 61, 91, 121, 151, 181, 211, 241, 271, 301, 331, 361, 391]:
        fig.savefig(svg_name, dpi=300)
